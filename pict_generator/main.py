...
fileext = ".jpg"
no_cv = False
export_path = './output/' + filename + '.svg'
draw_contours = True
draw_hatch = False
show_bitmap = False
resolution = 2048
...
def create_log_file():
    ...
    paths = get_paths('./output/' + filename + '.svg')
    ...
    for path in paths:
        if len(path) == 0:
            ...
            for i, point in enumerate(path):
                ...
                #if i == 0:
                    #actions.append('pdshiftxy '+ str(distance))
                #else:
                actions.append('shiftxy '+ str(distance))
                #actions.append('shiftxy '+ str(distance))
    ...
    file = open('./output/' + filename + '.log', 'w')
    file.write('\n'.join(str(x) for x in actions))
    file.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i','--input',dest='input_path',
        default='./images/' + filename + '.' + fileext,action='store',nargs='?',type=str,
        help='Input path')
