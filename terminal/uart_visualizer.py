...
def connect():
    ...
    #port = port_entry.get()
    #baud = baud_entry.get()
    serial_object = serial.Serial('COM6', '9600')
    ...
    #     print("Enter Baud and Port")
        # return
    serial_object.bytesize = serial.EIGHTBITS #number of bits per bytes
    serial_object.parity = serial.PARITY_NONE #set parity check: no parity
    serial_object.stopbits = serial.STOPBITS_ONE #number of stop bits
    serial_object.xonxoff = False     #disable software flow control
    serial_object.rtscts = False     #disable hardware (RTS/CTS) flow control
    serial_object.dsrdtr = False       #disable hardware (DSR/DTR) flow control
...
if __name__ == "__main__":
    ...
    """
    #frames
    frame_1 = Frame(height = 285, width = 575, bd = 3, relief = 'groove').place(x = 7, y = 5)
    frame_2 = Frame(height = 50, width = 575, bd = 3, relief = 'groove').place(x = 7, y = 300)
    text = Text(width = 69, height = 16)
    text.place(x = 15, y = 10)
    ...
    #contact = Label(text = "Logo").place(x = 250, y = 437)
    ...
    #Entry
    data_entry = Entry()
    data_entry.place(x = 150, y = 310, width=350,height=25)
    ...
    #button
    button1 = Button(text = "Send", command = send, width = 6).place(x = 510, y = 310)
    connect = Button(text = "Connect", command = connect).place(x = 15, y = 310)
    disconnect = Button(text = "Disconnect", command = disconnect).place(x =75, y = 310)
